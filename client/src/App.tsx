...
import { TunnelVulnerability } from './TunnelVulnerability';
...
export function App() {
  const [khulnasoftKey, setKhulnasoftKey] = React.useState("");
  const [khulnasoftSecret, setKhulnasoftSecret] = React.useState("");
  const [khulnasoftCSPMUrl, setKhulnasoftCSPMUrl] = React.useState("");
  ...
  const [uploadKhulnasoft, setUploadKhulnasoft] = React.useState(false);
  ...
  const [showUploadKhulnasoft, setShowUploadKhulnasoft] = React.useState("none")
  ...
  const [vulnerabilities, setVulnerabilities] = React.useState<TunnelVulnerability[]>([]);
  const [allVulnerabilities, setAllVulnerabilities] = React.useState<TunnelVulnerability[]>([]);
  ...
  async function createCacheVolume() {
    var success = true;
    await window.ddClient.docker.cli
      .exec("volume", ["create", "tunnel-docker-extension-cache"])
      .then((result: any) => {
        if (result.stderr !== "") {
          success = false;
        }
  ...
  async function triggerTunnel() {
    ...
    let commandParts: string[] = [
      ...
      "-v",
      "tunnel-docker-extension-cache:/root/.cache"
    ];

    if (uploadKhulnasoft && !SBOMOutput) {
      commandParts.push("-e");
      commandParts.push("TUNNEL_RUN_AS_PLUGIN=khulnasoft");
      commandParts.push("-e");
      commandParts.push("KHULNASOFT_KEY=" + khulnasoftKey);
      commandParts.push("-e");
      commandParts.push("KHULNASOFT_SECRET=" + khulnasoftSecret);
      commandParts.push("-e");
      commandParts.push("CSPM_URL=" + khulnasoftCSPMUrl);
    }


    commandParts.push("khulnasoft/tunnel");
    ...
    ({ stdout, stderr } = await runTunnel(commandParts, stdout, stderr));
  }

  async function runTunnel(commandParts: string[], stdout: string, stderr: string) {
    SendMetric("tunnel_scan_initiated", { imageName: scanImage });
            ...
            if (exitCode === 0) {
              if (uploadKhulnasoft && !SBOMOutput) {
                window.ddClient.desktopUI.toast.success(
                  `Results successfully uploaded to Khulnasoft`
                );
              }
              SendMetric("tunnel_scan_succeeded", { imageName: scanImage });
              processResult(res);
            } else {
              SendMetric("tunnel_scan_failed", { imageName: scanImage });
              window.ddClient.desktopUI.toast.error(
  ...
  const processResult = (res: any) => {
    ...
    for (let i = 0; i < results.Results.length; i++) {
      ...
      for (let j = 0; j < r.Vulnerabilities.length; j++) {
        ...
        vulns.push(new TunnelVulnerability(v));
        all += 1;
        switch (v.Severity) {
          case "CRITICAL":
            critical += 1;
  ...
  React.useEffect(() => {
    if (khulnasoftKey !== "" && khulnasoftSecret !== "") {
      setShowUploadKhulnasoft("");
    } else {
      setShowUploadKhulnasoft("none");
    }
  }, [khulnasoftKey, khulnasoftSecret]);

  React.useEffect(() => {
    window.ddClient.extension.vm.service.get("/credentials").then((value: any) => {
      setKhulnasoftKey(value.khulnasoft_key);
      setKhulnasoftSecret(value.khulnasoft_secret);
      setKhulnasoftCSPMUrl(value.khulnasoft_cspm_url);
      if (value.khulnasoft_key !== "" && value.khulnasoft_secret !== "" && value.khulnasoft_cspm_url !== "") {
        setLoggedIn(true);
      }
    }).catch((err: any) => {
      console.log(err);
    });
    SendMetric("tunnel_extension_opened", {});
  }, []);
    ...
    <DockerMuiThemeProvider>
      <div>
        ...
        <ConfigureCreds sx={{ float: 'right' }}
          ...
          setKhulnasoftKey={setKhulnasoftKey}
          khulnasoftSecret={khulnasoftSecret}
          setKhulnasoftSecret={setKhulnasoftSecret}
          khulnasoftCSPMUrl={khulnasoftCSPMUrl}
          setKhulnasoftCSPMUrl={setKhulnasoftCSPMUrl}
          loggedIn={loggedIn}
        ...
        <Box sx={{ clear: 'both' }}>
          <Welcome
            ...
            uploadKhulnasoft={uploadKhulnasoft}
            setUploadKhulnasoft={setUploadKhulnasoft}
            showUploadKhulnasoft={showUploadKhulnasoft}
