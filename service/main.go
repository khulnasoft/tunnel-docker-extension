...
import (
	...
	"github.com/khulnasoft/tunnel-docker-extension/service/internal/auth"
	"github.com/khulnasoft/tunnel-docker-extension/service/internal/socket"
	"github.com/labstack/echo"
	"github.com/sirupsen/logrus"
)

const credsFile = "/creds/.khulnasoft"

type Credentials struct {
	KhulnasoftKey     string `json:"khulnasoft_key"`
	KhulnasoftSecret  string `json:"khulnasoft_secret"`
	KhulnasoftCSPMUrl string `json:"khulnasoft_cspm_url"`
}

func main() {
	var socketPath = flag.String("socket", "/run/guest-services/plugin-tunnel.sock", "Unix domain socket to listen on")
...
func writeCredentials(ctx echo.Context) error {
	...
	validated, err := auth.ValidateCredentials(creds.KhulnasoftKey, creds.KhulnasoftSecret, creds.KhulnasoftCSPMUrl)
